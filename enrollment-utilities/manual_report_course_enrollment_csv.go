...
var AutomationSuffixes = []string{"Enrolled", "YearlyMember", "Renewal_Invitation",
    "PaymentFailed", "1_Month_Trial", "CollectingTestimonials",
    "Import_Rainmaker"}
...
var ExtraTagSuffixes = []string{"4ExtraMonths", "Cancelled", "FreeAccess",
    "NeverRenewed_Rainmaker", "Revoked", "TechGlitch", "YearlyMember",
    "Trial_Convert", "Trial_Dropout", "Enrolled_Trial"}
...
func myUsage() {
     ...
     fmt.Printf("Compare course enrollment between Teachable and ActiveCampaign for a" +
                ...
                "\tEWC: Experimenting With Color\n" +
                "\tSJC: Studio Journey Course\n" +
                "\tSJM: Studio Journey Masterclass\n" +
                "\tATC: Activating The Canvas\n" +
                "\tLYS: Light Your Creative Studio Like A Pro\n" +
                "\tBUNDLE_TAJC-EWC: Bundle: The Artist's Journey + Experimenting With Color\n" +
                "\tTAP_CHALLENGE: The Adjacent Possible: Creativity Challenge\n" +
                "\tTAPCIP: The Adjacent Possible: Creativity Immersion Program\n" +
                "\n")
     flag.PrintDefaults()
}
...
func main() {
    ...
    start := time.Now()
    firstStart := time.Now()
    // By <COURSE>_Enrolled tag
    tagName := courseAcronym + "_" + TagEnrolledSuffix
    if verbose > 0 {
    ...
    // By <COURSE>_Enrolled_Rainmaker tag
    var contactsWithRainmakerTag []ac.ListContactsContact
    rainmakerTagName := courseAcronym + "_" + TagRainmakerSuffix
    rainmakerTagStr := ""
    if includeRainmaker {
    ...
    if !quiet {
        log.Printf("Fetched %d contacts from AC with tag '%s'%s in: %v", len(allTagContacts), tagName,
            rainmakerTagStr, duration)
    }
    ...
    if !skipExtraTags {
        if !quiet {
            log.Printf("Fetching all contacts with %d extra tags in AC...",
                len(ExtraTagSuffixes))
        }
        start = time.Now()

        for _, v := range ExtraTagSuffixes  {
            // Get tag from AC
            extraTagName := fmt.Sprintf("%s_%s", courseAcronym, v)
            if verbose > 1 {
        ...
        if !quiet {
            log.Printf("Found %d contacts with %d extra tags in AC in: %v",
                totalExtrasCount, len(ExtraTagSuffixes), duration)
        }
    } // if !skipExtraTags
    ...
    */
    ...
    if includeRainmaker {
        // AC tag <COURSE>_Enrolled_Rainmaker
    ...
    if !skipExtraTags {
        ...
        if !quiet {
            log.Printf("%d contacts had extra tags for %s, but were not enrolled in AC:\n",
                len(extraTagContactsMissingInAc), courseAcronym)
        }
        if verbose > 0 {
    ...
    if !quiet {
        ...
        if includeRainmaker {
            log.Printf("%d students with %s_Enrolled_Rainmaker tag in AC but is not enrolled in AC:\n",
                len(rainmakerStudentsMissingInAc), courseAcronym)
            if verbose > 0 {
    ...
    t := time.Now()
    reportSpreadsheetName := fmt.Sprintf("%s_Enrolled%s%s_AC-Teachable_Comparison_" +
        "%d-%02d-%02dT%02d:%02d:%02d", courseAcronym, allStr, rainmakerStr, t.Year(),
    ...
    if !dryRun {
        ...
        headerRow := []string{"Email (from both lists):", "In Teachable?",
                              fmt.Sprintf("In ActiveCampaign (has %s tag)?", tagName) }
        if includeRainmaker {
        ...
        formatEndColumnIndex := int64(3)
        if includeRainmaker {
            formatEndColumnIndex = int64(4)
        }
...
func (s *CourseStudent) String() string {
    return fmt.Sprintf("%s\tac=%t, teachable=%t, rainmaker=%t, automations=%+v, tags=%+v",
        s.Email, s.IsInAc, s.IsInTeachable, s.IsFromRainmaker, s.AcAutomations, s.AcExtraTags)
}
