...
func main() {
	...
	// Iterate all Stripe customers and check if they're founders
	...
	founderStatusById := map[string]*(sj.StudentStatus){}
	haveError := false
	...
	// Update the founders
	log.Printf("Updating %d Stripe foundation customers...\n", foundersCount)
	if cancelCompleted {
		log.Printf("Also prompting for canceling completed foundation customers.\nWARNING: Be sure that the cancelation webhook is turned off in Zapier.\n")
	}
	dryRunStr := "Dry-run: "
	if !dryRun {
		dryRunStr = ""
	...
	//bar.Increment()
	for _, c := range foundationCustomers {
		s := founderStatusById[c.ID]
		if verbose > 1 {
		...
		if cancelCompleted && s.IsBillingComplete && s.IsBillingActive {
			msg := fmt.Sprintf("%sCancel completed founding member \"%s\" (%s) with Stripe ID \"%s\"",
